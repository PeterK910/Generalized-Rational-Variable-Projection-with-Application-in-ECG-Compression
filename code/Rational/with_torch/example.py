...
from Rational.with_torch.hyp_mdpso import hyp_mdpso
...
from Rational.with_torch.Hyperbolic_operators.draw_unitcircle import draw_unitcircle
...
signal = signal[0]
M = len(signal)
signal = torch.reshape(torch.tensor(signal), (1, M))  # signal should be a row vector.
signal = signal[0]
...
mpoles = periodize_poles(multiply_poles(p, m, allow_unique=True), 1)
aprx = torch.real(mt_generate(M, mpoles, c))
...
print(f'Inverse pole configuration in the best dimension: m=({m.tolist()})')
...
#plt.savefig("results/rational_withtorch_signal_approx.png")
plt.show()
